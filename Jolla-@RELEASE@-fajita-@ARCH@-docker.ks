# DisplayName: Jolla fajita/@ARCH@ (release) 5.0.0.67+docker
...
repo --name=local-kernel --baseurl=file:///work/rpmrepo --no-gpgcheck

%packages
patterns-sailfish-device-configuration-fajita
docker-kernel-oneplus6t
%end
...
### end 50_os-release
### begin 90_docker-boot
# 用自定义 Docker boot 镜像覆盖 adaptation 包提供的 hybris-boot.img
if [ -f "/work/boot_a-docker-v2.img" ]; then
    mkdir -p $INSTALL_ROOT/boot
    cp -f /work/boot_a-docker-v2.img $INSTALL_ROOT/boot/hybris-boot.img
    echo "[docker-boot] 已用 boot_a-docker-v2.img 覆盖 /boot/hybris-boot.img"
else
    echo "[docker-boot] 警告: /work/boot_a-docker-v2.img 不存在，使用 adaptation 默认 boot 镜像"
fi
### end 90_docker-boot
...
# Pack updater .zip
pushd updater # sfe-$DEVICE-$RELEASE_ID
